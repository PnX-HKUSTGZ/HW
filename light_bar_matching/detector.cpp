...
std::vector<Armor> Detector::matchLights(const std::vector<Light> & lights)
{
  ...
  for (auto light_1 = lights.begin(); light_1 != lights.end(); light_1++) {
    for (auto light_2 = light_1 + 1; light_2 != lights.end(); light_2++) {
      // std::cout << light_1->color << light_2->color << std::endl;
      ...
      std::string armor_type_str = ARMOR_TYPE_STR[static_cast<int>(type) - 1];

      // std::cout << armor_type_str << std::endl;
...
ArmorType Detector::isArmor(const Light & light_1, const Light & light_2)
{
  ...
  // std::cout << light_ratio_ok << center_distance_ok << angle_ok << std::endl;
  // Judge armor type
  ArmorType type;
  if (is_armor) {
